...
- **响应式布局**：完美适配桌面、平板、手机
- **亮暗主题切换**：支持浅色/深色/自动模式，自动模式 06:00–18:00 亮色
- **本地历史记录**：浏览器 localStorage 自动保存对话，支持搜索、重命名、置顶、JSON 导出
- **消息操作**：复制、重试、语音播放、删除、点赞/点踩
- **Token 用量统计**：后台实时统计各模型调用次数和 token 消耗
- **后台管理系统**：JWT 认证、仪表盘统计、配置在线编辑、日志查看、限流管理
...
├── user.webp               用户头像
├── admin/                  后台管理系统
│   ├── config.php          后台配置（密码/JWT密钥）
│   ├── auth.php            JWT认证 + 工具函数
│   └── index.php           后台路由（登录/仪表盘/API）
└── file/                   运行时目录（自动创建）
    ├── .htaccess           目录访问保护
...
- 管理员密码使用 bcrypt 哈希存储
- JWT 令牌认证（HS256），12 小时过期，HttpOnly Cookie
- CSRF 防护：JWT 内嵌随机令牌，验证所有管理 API 调用
- `file/.htaccess` 禁止目录浏览，仅允许指定文件类型访问
- 配置文件保存自动备份
...
访问地址：`/admin/index.php`，默认密码：`admin123`

### 1. 功能矩阵

| 功能 | 说明 |
|------|------|
| JWT 登录 | bcrypt 密码验证 + HS256 令牌 |
| 统计仪表盘 | 用户记忆数、文件数、限流状态、Token 用量 |
| API 配置编辑 | 直接编辑端点、密钥、超时、限流等参数并保存 |
| 模块配置编辑 | 可编辑每个 AI 模块的模型名、Tokens、系统提示词 |
| 系统日志查看 | 实时查看当天 error/security 日志 |
| 限流管理 | 查看活跃限流记录，一键清理 |
| 记忆管理 | 按用户清理或全部清理 |
| 亮暗主题 | 自动跟随系统，支持手动切换 |
...
```
将输出的哈希值更新到 `admin/config.php` 的 `ADMIN_PASSWORD_HASH`。

生产环境建议修改 `ADMIN_JWT_SECRET` 为随机字符串。

### 3. Token 用量统计

系统自动记录每次 API 调用的 token 消耗（prompt/completion/total），存储在 `file/log/token_usage.json`。
后台仪表盘展示：
- 今日 Token / 累计 Token
- 今日平均 / 累计平均（每次调用）
- 今日调用次数
- 分模型明细（调用次数、prompt、completion、合计、平均）
...
**A：** 浏览器端本地历史记录会自动保存最近对话。刷新页面或重新打开时自动恢复最近一次对话。如需管理历史，点击顶栏"历史记录"按钮，支持搜索、重命名、置顶、JSON 导出。对话记忆在服务端基于 uid 存储，支持跨会话延续上下文。
